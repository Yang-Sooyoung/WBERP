...
$(document).ready(function(){
	...
	$("#pviewBar,#pviewCol, #pviewLine, #pviewPie").on("click", function(){
		...
		$.ajax({
			...
			url : "testAjax", // {키:값, 키:값}// ""데이터가 넘어갈 주소 (결과를 받을 주소)
			dataType : "json",
			data : params, // 보내는 데이터
...
<body>
	...
	<div class="contents_header">
		<div class="contents_header_txt">위젯 수정</div>
	</div>
	<div class="contents">
		<!-- 여기에 내용을 작성하시오. -->
		<!-- 화면전환 시 아래와 같이 폼을 구성하시오. -->
		<form action="widgDetail" id="actionForm" method="post">
		<!-- 아래 두개 필수 -->
			<input type="hidden" id="top" name="top" value="${param.top}" />
			<input type="hidden" id="menuNo" name="menuNo" value="${param.menuNo}" />
			<input type="hidden" id="DASB_NO" name="DASB_NO" value="${param.DASB_NO}" />
			<input type="hidden" id="WIDG_NO" name="WIDG_NO" value="${param.WIDG_NO}" />
			...
			장표선택 :
				...
				<br/>
			장표번호 : <input type="text" readonly="readonly" name="ATTCH_NO" id="ATTCH_NO" value="${list.ATTCH_NO }"/><br/>
			장표쿼리: <textarea class="attch_query" rows="10" cols="20" name="ATTCH_QUERY"id="ATTCH_QUERY">${list.ATTCH_QUERY }</textarea><br/><br/><br/>
			...
			차트타입 : <input type="text" name="CHART_TYPE" id="CHART_TYPE" value="${list.CHART_TYPE}"/><br/>
			위젯순서 : <input type="text" name="WIDG_ORDER" id="WIDG_ORDER" value="${list.WIDG_ORDER}"/><br/>
