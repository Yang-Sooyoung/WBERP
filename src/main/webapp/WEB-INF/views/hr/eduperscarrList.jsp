...
$(document).ready(function() {
	...
	$("#addBtn").on("click", function() {
		$("#actionForm").attr("action", "eduperscarrAdd");
		$("#actionForm").submit();
	});
	
	$("tbody").on("click", "tr", function() {
		$("#edu_pers_carr_no").val($(this).attr("name"));
		$("#actionForm").submit();
...
function drawList(list, cnt) {
	...
	if(cnt == 0) {
		...
		for(var i = 0 ; i < list.length ; i++) {
			html += "<tr name=\"" + list[i].EDU_PERS_CARR_NO + "\">";
			html += "<td>" + list[i].CARR_NM + "</td>";
			html += "<td>" + list[i].START_DAY + " ~ " + list[i].END_DAY +  "</td>";
			html += "<td>" + list[i].AGNC_NM + "</td>";
	/* 	if("${sMno}" == list[i].MNO) {
...
<body>
	...
	<div class="contents_header">
		<div class="contents_header_txt">경력 조회</div>
	</div>
	<div class="contents">
		<!-- 여기에 내용을 작성하시오. -->
		<form action="#" method="post" id="actionForm">
			...
			<input type="hidden" name="edu_pers_carr_no" id="edu_pers_carr_no" value="${edu_pers_carr_no}" />
			<input type="hidden" name="page" id="page" value="${page}" />
		<div class="default_search_area" style="width : 723px;">
			<select name ="searchGbn">
				<option value="0">경력명</option>
