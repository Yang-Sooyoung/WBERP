...
<meta charset="UTF-8">
<!-- 페이지에 맞추어 타이틀 작업 -->
<title>기업은행 WBERP - 근태코드 조회</title>
<style type="text/css">
...
.ated_detail tbody tr:hover {
	background-color: inherit !important;
	color: inherit !important;
}
.ated_add tbody tr:hover {
	background-color: inherit !important;
	color: inherit !important;
}
</style>
...
$(document).ready(function() {
	...
	$(".detail_btn_area").on("click", "#listBtn", function() {
		$("#ch1").show(); /* 제목  */
 		...
 		$("#ch3").hide();
 		$("#ch4").hide();
 		$(".ated_list").css('display', 'table'); /* 테이블  */
 		$(".ated_list tbody").css('display', '');
		$(".ated_detail").css('display', 'none');
 		$(".ated_add").css('display', 'none');
	...
	/* 상세조회 */
	$(".ated_list").on("click", "tbody tr", function() {
		$("#ated_code_no").val($(this).attr("name")); /* 글번호 */
 		$("#ch1").hide(); /* 제목  */
		...
		$(".ated_list").css('display', 'none'); /* 테이블  */
		$(".ated_detail").css('display', 'table');
 		$(".ated_add").css('display', 'none');
	...
	/* 등록  */
	$(".search_btn_area").on("click.addBtn", "#addBtn", function() {
		$("#ch1").hide(); /* 제목  */
 		$("#ch2").hide();
	...
	$(".detail_btn_area").off("click.saveBtn").on("click.saveBtn", "#saveBtn", function() {
 		if ($.trim($("#tx3").val()) == "") {
			...
			alert("근태코드설명을 입력해주세요.");
			$("#tx4").focus();
		} else {
			makeBtn2Popup(1, "근태코드 등록", "등록하시겠습니까?", true, 400, 200, 
				      null, "저장", function() {
					     saved1();
					     closePopup(1);		
    		 		  }, "취소", function() {
 	...
 	/* 수정  */
	$(".detail_btn_area").on("click.updateBtn", "#updateBtn", function(){
 		$("#ch1").hide(); /* 제목  */
 		$("#ch2").hide();
 		$("#ch3").hide();
	...
	$(".detail_btn_area").off("click.saveBtn").on("click.saveBtn", "#saveBtn", function() {
 		if ($.trim($("#tx1").val()) == "") {
			...
			makeBtn2Popup(1, "근태코드 수정", "수정하시겠습니까?", true, 400, 200, 
					      null, "저장", function() {
						     saved2();
						     closePopup(1);	
	      		 		  }, "취소", function() {
...
function drawList(list, cnt) {
	...
	if (cnt == 0) {
		...
		for (var i = 0; i < list.length; i++) {
			html += "<tr name=\"" + list[i].ATED_CODE_NO + "\">";
			html += "<td>" + list[i].ATED_CODE_NO + "</td>";
...
function reloadDetail() {
	...
	$.ajax({
		type : "post",
		url : "atedCodeDetailContAjax",
		dataType : "json", // {키:값, 키:값}
		data : params, // 보낼데이터
		success : function(res) { // 성공 시 해당 함수 실행. 결과는 result로 받겠다.
			drawDetail(res.atedcode);
		},
		error : function(req, status, error) {
			console.log("code : " + req.status);
			console.log("message : " + req.responseText);
...
function drawDetail(atedcode) {
	...
	html += "</colgroup>";
	html += "<tr name=\"" + atedcode.ATED_CODE_NO + "\">"
	html += "<th class=\"top2\">" + "근태코드명" + "</th>";
	html += "<td class=\"cont1\">"
			+ atedcode.ATED_CODE_NM
			+ "</td>";
	html += "<td class=\"cont2\">"
			+ "<input type=\"text\" id=\"tx1\" name=\"ated_code_nm2\" value=\"" + atedcode.ATED_CODE_NM + "\"  placeholder=\"내용을 입력해주세요.\" />"
			+ "</td>";
	html += "</tr>";
	html += "<tr name=\"plag\">";
	html += "<th class=\"top2\">" + "근태코드설명" + "</th>";
	html += "<td class=\"cont1\">"
			+ atedcode.ATED_CODE_EXPN
			+ "</td>";
	html += "<td class=\"cont2\">"
			+ "<input type=\"text\" id=\"tx2\" name=\"ated_code_expn2\" value=\"" + atedcode.ATED_CODE_EXPN + "\"  placeholder=\"내용을 입력해주세요.\" />"
			+ "</td>";
	html += "</tr>";

	$(".ated_detail tbody").html(html);
}
</script>
</head>
<body>
	...
	<div class="contents_header">
		<div class="contents_header_txt" id="ch1">근태코드 조회</div>
		<div class="contents_header_txt" id="ch2">근태코드 상세조회</div>
		<div class="contents_header_txt" id="ch3">근태코드 등록</div>
		<div class="contents_header_txt" id="ch4">근태코드 수정</div>
	</div>
	<div class="contents">
		<!-- 여기에 내용을 작성하시오. -->
		<form action="#" method="post" id="actionForm">
			...
			<div class="default_search_area">
				<select name="searchGbn" id="searchGbn">
					<option value="0">근태코드명</option>
					<option value="1">근태코드설명</option>
				</select> 
			...
			<div class="default_board">
				<table class="ated_list">
				</table>
				<table class="ated_detail">
					<tbody>
					</tbody>
				</table>
				<table class="ated_add">
					<colgroup>			
						<col width="150px">
						<col width="300px">
					</colgroup>
					...
					<tr>
						...
						<td class="cont2">
				 			<input type="text" id="tx3" name="ated_code_nm1" placeholder="내용을 입력해주세요." />
				 		</td>
					</tr>
					<tr>
						<th class="top2">근태코드설명</th>
						<td class="cont2">
				 			<input type="text" id="tx4" name="ated_code_expn1"  placeholder="내용을 입력해주세요." />
				 		</td>
					</tr>
					</tbody>
				</table>
