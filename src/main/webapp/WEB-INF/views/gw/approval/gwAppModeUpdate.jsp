...
<script type="text/javascript">
	...
	$(".detail_btn_area").on("click", "#updateBtn", function() {
		$("#modecont").val(CKEDITOR.instances['modecont'].getData());
		...
		if($.trim($("#modetitle").val())=="" || $.trim($("#modeexpn").val())=="" || $.trim($("#modecont").val())=="") {
			makeBtn1Popup(1, "", "필수내용을 모두 작성해야합니다.", true, 400, 200,
				...
				$.ajax({
					type : "post",
					url : "AppModeUpdateAjax", //양식수정Ajax
					dataType : "json",
					data : params,
					success : function(res) {
						if (res.res == "SUCCESS") {
							makeBtn1Popup(1, "", "수정되었습니다.", true, 400, 200, null, "확인", function() { //양식수정성공시 팝업창
								closePopup(1);
...
<body>
	...
	<div class="contents_header">
		<div class="contents_header_txt">양식수정</div>
	</div>
	<div class="contents">
		<!-- 여기에 내용을 작성하시오. -->
		...
		<div class="underbox">
			<div class="underline">
				...
				<div class="con1">
					<input type="text" name="modetitle" id="modetitle" value="${mode.APRV_MODE_NM}"/>
				</div>
			</div>
			<div class="underline">
				<div class="line1">양식설명</div>
				<div class="con3">
					<input type="text" name="modeexpn" id="modeexpn" value="${mode.APRV_MODE_EXPN}" />
				</div>
			</div>
			<div class="underline">
				<div class="line1">내용</div>
				<div class="con3">
				<textarea name="modecont" id="modecont">${mode.APRV_MODE_MODE}</textarea>
				</div>
			</div>
		</div>
		</form>
