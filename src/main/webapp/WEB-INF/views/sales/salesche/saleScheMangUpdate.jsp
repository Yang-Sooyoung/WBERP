...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#cancelBtn").on("click", function() {
			makeBtn2Popup(1, "수정 취소 팝업", "취소하시겠습니까?", true, 400, 200,
					null, "확인", function() {
		...
		function saleScheUpdateAjax(){
			var params = $("#actionForm").serialize(); //serialize : form안에 있는 데이터를 문자열로 바꾼다.
			
			$.ajax({ //제이쿼리에 있는 아작스
				type : "post", //주고 받는 방식, 주로 포스트 방식으로 주고 받음
				url : "saleScheUpdateAjax",//data를 url 주소에 보낸다
...
<body>
	...
	<c:import url="/topLeft">
		...
		<div class="contents_header">
			<div class="contents_header_txt">영업 일정 수정</div>
	...
	<form action="#" id="actionForm" method="post">
		...
		<input type="hidden" name="entpno" id="entpno" value="${data.ENTP_NO}"/>
		<input type="hidden" name="custno" id="custno" value="${data.CUST_NO}"/>
		<input type="hidden" name="godsno" id="godsno" value="${data.GODS_NO}" />
		<input type="hidden" name="salescheno" id="salescheno" value="${param.salescheno }" />
			<input type="hidden" name="scheno" id="scheno" value="${data.SCHE_NO }" />
			<input type="hidden" name="atchfilenm" id="atchfilenm" value="${data.ATCH_FILE_NM}" />
		<input type="hidden" name="salescheatchfileno" value="${param.salescheatchfileno}"/>
		...
		<input type="hidden" name="searchTxt3Hidden" id="searchTxt3Hidden" value="${param.searchTxt3Hidden}" />
				...
				<div class="underline">
					...
					<div class="add_con1">
						<input type="text" id="schenm" name="schenm" value="${data.SCHE_NM}"/>
					</div>
					<div class="add_line1">날짜 *</div>
					<div class="add_con1">
						<input type="date" id="schestartday" name="schestartday" value="${data.SCHE_START_DAY}"/>
					</div>
				</div>
				<div class="underline">
					<div class="add_line1">고객사 명 *</div>
					<div class="add_con1">
						<input type="text" disabled="disabled" id="entpnm" name="entpnm" value="${data.ENTP_NM}" />
					</div>
					<div class="add_line1">시간 *</div>
					<div class="add_con1">
						<select class="time_txt1" id="schestartdaytimeh" name="schestartdaytimeh">
							<option value="">선택</option>
							<option value="00" <c:if test="${data.SCHE_START_DAY_TIME_H==00}"> selected </c:if>>00</option>
							<option value="01" <c:if test="${data.SCHE_START_DAY_TIME_H==01}"> selected </c:if>>01</option>
							<option value="02"<c:if test="${data.SCHE_START_DAY_TIME_H==02}"> selected </c:if>>02</option>
							<option value="03"<c:if test="${data.SCHE_START_DAY_TIME_H==03}"> selected </c:if>>03</option>
							<option value="04"<c:if test="${data.SCHE_START_DAY_TIME_H==04}"> selected </c:if>>04</option>
							<option value="05"<c:if test="${data.SCHE_START_DAY_TIME_H==05}"> selected </c:if>>05</option>
							<option value="06"<c:if test="${data.SCHE_START_DAY_TIME_H==06}"> selected </c:if>>06</option>
							<option value="07"<c:if test="${data.SCHE_START_DAY_TIME_H==07}"> selected </c:if>>07</option>
							<option value="08"<c:if test="${data.SCHE_START_DAY_TIME_H==08}"> selected </c:if>>08</option>
							<option value="09"<c:if test="${data.SCHE_START_DAY_TIME_H==09}"> selected </c:if>>09</option>
							<option value="10"<c:if test="${data.SCHE_START_DAY_TIME_H==10}"> selected </c:if>>10</option>
							<option value="11"<c:if test="${data.SCHE_START_DAY_TIME_H==11}"> selected </c:if>>11</option>
							<option value="12"<c:if test="${data.SCHE_START_DAY_TIME_H==12}"> selected </c:if>>12</option>
							<option value="13"<c:if test="${data.SCHE_START_DAY_TIME_H==13}"> selected </c:if>>13</option>
							<option value="14"<c:if test="${data.SCHE_START_DAY_TIME_H==14}"> selected </c:if>>14</option>
							<option value="15"<c:if test="${data.SCHE_START_DAY_TIME_H==15}"> selected </c:if>>15</option>
							<option value="16"<c:if test="${data.SCHE_START_DAY_TIME_H==16}"> selected </c:if>>16</option>
							<option value="17"<c:if test="${data.SCHE_START_DAY_TIME_H==17}"> selected </c:if>>17</option>
							<option value="18"<c:if test="${data.SCHE_START_DAY_TIME_H==18}"> selected </c:if>>18</option>
							<option value="19"<c:if test="${data.SCHE_START_DAY_TIME_H==19}"> selected </c:if>>19</option>
							<option value="20"<c:if test="${data.SCHE_START_DAY_TIME_H==20}"> selected </c:if>>20</option>
							<option value="21"<c:if test="${data.SCHE_START_DAY_TIME_H==21}"> selected </c:if>>21</option>
							<option value="22"<c:if test="${data.SCHE_START_DAY_TIME_H==22}"> selected </c:if>>22</option>
							<option value="23"<c:if test="${data.SCHE_START_DAY_TIME_H==23  }"> selected </c:if>>23</option>
						</select> : <select class="time_txt2" id="schestartdaytimem" name="schestartdaytimem">
							<option value="">선택</option>
							<option value="00"<c:if test="${data.SCHE_START_DAY_TIME_M==00}"> selected </c:if>>00</option>
							<option value="30"<c:if test="${data.SCHE_START_DAY_TIME_M==30}">selected </c:if>>30</option>
						</select> ~ <select class="time_txt3" id="scheenddaytimeh" name="scheenddaytimeh">
							<option value="">선택</option>
							<option value="00"<c:if test="${data.SCHE_END_DAY_TIME_H==00}">selected </c:if>>00</option>
							<option value="01"<c:if test="${data.SCHE_END_DAY_TIME_H==01}">selected </c:if>>01</option>
							<option value="02"<c:if test="${data.SCHE_END_DAY_TIME_H==02}">selected </c:if>>02</option>
							<option value="03"<c:if test="${data.SCHE_END_DAY_TIME_H==03}">selected </c:if>>03</option>
							<option value="04"<c:if test="${data.SCHE_END_DAY_TIME_H==04}">selected </c:if>>04</option>
							<option value="05"<c:if test="${data.SCHE_END_DAY_TIME_H==05}">selected </c:if>>05</option>
							<option value="06"<c:if test="${data.SCHE_END_DAY_TIME_H==06}">selected </c:if>>06</option>
							<option value="07"<c:if test="${data.SCHE_END_DAY_TIME_H==07}">selected </c:if>>07</option>
							<option value="08"<c:if test="${data.SCHE_END_DAY_TIME_H==08}">selected </c:if>>08</option>
							<option value="09"<c:if test="${data.SCHE_END_DAY_TIME_H==09}">selected </c:if>>09</option>
							<option value="10"<c:if test="${data.SCHE_END_DAY_TIME_H==10}">selected </c:if>>10</option>
							<option value="11"<c:if test="${data.SCHE_END_DAY_TIME_H==11}">selected </c:if>>11</option>
							<option value="12"<c:if test="${data.SCHE_END_DAY_TIME_H==12}">selected </c:if>>12</option>
							<option value="13"<c:if test="${data.SCHE_END_DAY_TIME_H==13}">selected </c:if>>13</option>
							<option value="14"<c:if test="${data.SCHE_END_DAY_TIME_H==14}">selected </c:if>>14</option>
							<option value="15"<c:if test="${data.SCHE_END_DAY_TIME_H==15}">selected </c:if>>15</option>
							<option value="16"<c:if test="${data.SCHE_END_DAY_TIME_H==16}">selected </c:if>>16</option>
							<option value="17"<c:if test="${data.SCHE_END_DAY_TIME_H==17}">selected </c:if>>17</option>
							<option value="18"<c:if test="${data.SCHE_END_DAY_TIME_H==18}">selected </c:if>>18</option>
							<option value="19"<c:if test="${data.SCHE_END_DAY_TIME_H==19}">selected </c:if>>19</option>
							<option value="20"<c:if test="${data.SCHE_END_DAY_TIME_H==20}">selected </c:if>>20</option>
							<option value="21"<c:if test="${data.SCHE_END_DAY_TIME_H==21}">selected </c:if>>21</option>
							<option value="22"<c:if test="${data.SCHE_END_DAY_TIME_H==22}">selected </c:if>>22</option>
							<option value="23"<c:if test="${data.SCHE_END_DAY_TIME_H==23}">selected </c:if>>23</option>
						</select> : <select class="time_txt4" id="scheenddaytimem" name="scheenddaytimem">
							<option value="" >선택</option>
							<option value="00" <c:if test="${data.SCHE_END_DAY_TIME_M==00}">selected </c:if>>00</option>
							<option value="30" <c:if test="${data.SCHE_END_DAY_TIME_M==30}">selected </c:if>>30</option>
						</select>
					</div>
				</div>
				<div class="underline">
					...
					<div class="add_con1">
						<input type="text" disabled="disabled" id="custnm" name="custnm" value="${data.CUST_NM}"/>
						<input type="button" id="custnmBtn" value="검색" />
					</div>
					<div class="add_line1">전화번호 *</div>
					<div class="add_con1">
						<input type="text" disabled="disabled" id="custcell" value="${data.CELL_NO}"/>
					</div>
				</div>
				<div class="underline">
					<div class="add_line1">장소 *</div>
					<div class="add_con3">
						<input class="pl_txt" name="scheloct" type="text" id="scheloct" value="${data.SCHE_LOCT}"/>
				...
				<div class="underline">
					...
					<div class="add_con1">
						<input type="text" id="godsnm" name="godsnm" value="${data.GODS_NM}"/>
						<input type="button" id="godsnmBtn" value="검색" />
					</div>
					<div class="add_line1">영업 목적</div>
					<div class="add_con1">
						<input type="text" id="salepurp"  name="salepurp" value="${data.SALE_PURP}"/>
					</div>
				</div>
					...
					<div class="underline">
						...
						<div class="add_con3">
							<textarea rows="8" cols="130" id="actvcont" name="actvcont">${data.ACTV_CONT}</textarea>
						</div>
					</div>
					<c:if test="${!empty data.ATCH_FILE_NM}">
						<div class="underline" id="underline1">
					<div class="add_line1">첨부파일</div>
					<div class="add_con3">
						<div class="add_file_txt">
						<div id="fileDiv">
							${data.ATCH_FILE_NM}<input type="button" value="첨부파일 삭제" id="fileDelBtn"/>
							</div>
						</div>
					</div>
				</div>
				...
				<div class="underline" id="underline2">
					<div class="add_line1">첨부파일</div>
					<div class="add_con3">
						<div class="add_file_txt">
